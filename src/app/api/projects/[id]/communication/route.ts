...
// GET /api/projects/[id]/communication - Get conversation/communication feed with filters
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch communication feed' },
      { status: 500 }
    )
  }
}

// POST /api/projects/[id]/communication - Send message (user override or agent communication)
export async function POST(
  ...
  try {
    ...
    // Track human override if from user
    if (body.fromRole === 'user' || !body.fromAgentId) {
    ...
    log.push({
      ...
      event: 'communication_message_sent',
      details: {
